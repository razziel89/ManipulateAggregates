...
@package ManipulateAggregates.energyscan.ansilliary
"""
...
try:
    ...
    logger.warning("Could not import ..collection.write.print_dx_file as c_print_dx_file")
try:
...
def init_hashing(depth,width,alg):
    ...
    @a depth * @a width is not larger than the number of hexadecimal places of
...
def print_dx_file(prefix,hash,dictionary,values,comment):
    ...
    c_print_dx_file(filename,counts,org,delx,dely,delz,values,comment=comment,gzipped=gzipped,formatstring="14.13e")
...
def prepare_molecules(mol1,mol2,aligned_suffix="",save_aligned=False,align=True):
    """Prepare two molecules for the scanning procedure. First, align @a mol1
    and @a mol2 with their centers to @a org and their third and second main
    axes with [1,0,0] and [0,1,0], respectively.  Then, append mol2 to mol1 and
    enable tagging to allow easily moving @a mol2 with one command.  The
    ...
    Returns:
        an OBAggregate object that contains @a mol1 and @a mol2
    """
