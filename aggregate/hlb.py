...
There are two methods supported by this subsubmodule:
  -# simple: Scan the molecule with a plane that is allowed to cut midway
    ...
    side.
  -# complex: Scan the molecule with a plane that is allowed to cut midway
...
@package ManipulateAggregates.aggregate.hlb
"""
...
def part_molecule(structure,normal_vector,coordinate):
    ...
    Returns:
        a list of indices of all elements of @a structure that are on the side
        of the plane (defined by @a normal_vector and @a coordinate) to which
        the normal vector points

    Args:
        structure: (list of vectors) Cartesian coordinates that shall be separated by the plane
...
def list_min(l,key=lambda x:x):
    ...
    Kwargs:
        key: (function of 1 argument) this function is applied to every element
            of @a l to get an element that the "min" function understands
    """
...
def pseudo_energy_simple(structure,names,normal_vector,point_in_plane):
    ...
    Returns:
        ...
        defined by @a normal_vector and @a point_in_plane cutting the molecule
        defined by @a structure and @a names)
...
def pseudo_energy_complex(max_potential,point,vector,centers,potential,weights):
    ...
    Args:
        max_potential: (float) the maximum value of @a potential
...
def get_HLB_simple(mol,thetas,phis,no_hydrogen,
        relative_angles=False,pseudo_energy_func=pseudo_energy_simple):
    ...
    Args:
        ...
        thetas: (list of floats) same as @a phis but for the theta angle of
    ...
    if no_hydrogen:
        ...
        structure_no_H = array([structure[i] for i in range(0,len(names)) if names[i]!="H"])
    else:
...
def get_HLB_complex(mol, thetas, phis, no_hydrogen, nr_refinements,dependence,relative_angles=False):
    ...
    Returns:
        ...
        floats) is the point in the plane (required for Hessian normal form)
    """
    corners,face_indices,normals = mol.get_vdw_surface(nr_refinements=nr_refinements,shrink_factor=0.95,vdwscale=1.0)
    ...
    surfaceareas = array([norm(cross(face[1]-face[0],face[2]-face[0])) for face in corners[face_indices]])/2.0
    ...
    if dependence == 'dependent':
        ...
        abs_potential = fabs(abs_potential*greater_zero/max_potential) + fabs(abs_potential*smaller_zero/min_potential)
    else:
    ...
    if len(bondmap)==0:
        raise RuntimeError("Number of bonds through which cutting would be allowed is 0!")
    ...
    for point,vectors in test_generator:
        for vector in vectors:
            energy=pseudo_energy_complex(max_potential,point,vector,centers,abs_potential,surfaceareas)
...
def compute(mol,phi_step_degree=5.0,theta_step_degree=5.0,theta_range_degree=10.0,no_hydrogen=True,
        method="simple",write=False,nr_refinements=1,dependence='independent'):
    ...
    thetas=concatenate((arange(-theta_range,theta_range,theta_step),arange(pi+theta_range,pi-theta_range,-theta_step)))
    ...
    if method == "simple":
        (hlb_value,normal_vector,point_in_plane)=get_HLB_simple(mol,thetas,phis,no_hydrogen,relative_angles=True)
    elif method == "complex":
        (hlb_value,normal_vector,point_in_plane)=get_HLB_complex(mol,thetas,phis,no_hydrogen,nr_refinements,dependence,relative_angles=True)
    
    if write:
        #print both parts if desired
        filename = mol.info.get("name","part")
        fileformat = mol.info.get("format","xyz")
        mol.part_aggregate(normal_vector,point_in_plane,side='left').write("hydrophilic_"+filename,fileformat=fileformat)
        mol.part_aggregate(normal_vector,point_in_plane,side='right').write("lipophilic_"+filename,fileformat=fileformat)
