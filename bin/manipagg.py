...
def __closer(parts,stepsize,config=None): #2 [#1]
    ...
    if _nn(config):
        ...
        _le(config,2)
        CURRENTAGG.move_closer(parts[0],parts[1],stepsize=step,vdw_factor=config[0],vdw_added=config[1],vec=None)
    else:
...
def __closer_vec(parts,vector,stepsize,config=None): #3 [#1]
    ...
    if _nn(config):
        config = list(map(float,config.split(",")))
        _le(config,2)
        CURRENTAGG.move_closer(parts[0],parts[1],stepsize=step,vdw_factor=config[0],vdw_added=config[1],vec=vector)
    else:
...
def __colorscale(start,end=None): #1 [#1]
    ...
    if _nn(end):
        ...
        raise TypeError("The first argument is no special keyword but no second argument given.")
