...
def single_data(filename,header=None,dir="",progress=False,save_all_mos=False,points=80,cutoff=7.0):
    ...
    basis,(allMOsalpha,allOCCsalpha),(allMOsbeta,allOCCsbeta)  = get_MOs_and_basis(filename,filetype="molden",spins='both')
    #determine occupied orbitals
    MOsalpha  = [mo for mo,occ in zip(allMOsalpha,allOCCsalpha) if occ>minocc]
    MOsbeta   = [mo for mo,occ in zip(allMOsbeta,allOCCsbeta)   if occ>minocc]
    OCCsalpha = [occ for occ in allOCCsalpha if occ>minocc]
    OCCsbeta  = [occ for occ in allOCCsbeta  if occ>minocc]
    #determine LUMOs (index starts at 0, so the length is always maxindex+1)
    LUMOalpha = allMOsalpha[len(MOsalpha)]
    LUMObeta  = allMOsalpha[len(MOsbeta)]
...
def postprocess_multiple(total_1,total_2,MOalpha_1,MObeta_1,MOalpha_2,MObeta_2,dir="",type="kation"):
    ...
    MOalpha_1, MObeta_1: str
        Names of the dx-files that contain the HOMO/LUMO density of
    ...
    type: str
        ...
        kation or anion. If true, the MOs specified have to be
        the HOMOs, otherwise the LUMOs.
    """
    print >>sys.stderr,"DEBUG: started check of orbital character"
    if len(dir)>0:
        if not dir.endswith("/"):
