...
class Molecule(object):
    ...
    def draw(self, show=False, filename=None, update=False, usecoords=False):
        """Create a 2D depiction of the molecule.

        Optional parameters:
          show -- display on screen (default is False, True not implemented!)
          filename -- write to file (default is None, not None not implemented)
        ...
        if filename:
            ...
            if sys.platform[:3] == "cli" and show:
                raise NotImplementedError("Value show==True not supported")
